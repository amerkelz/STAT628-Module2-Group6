...
for (var in variables_new) {
  ...
  rmse <- round(sqrt(mean((actuals - predictions)^2)),2)
  r_squared_new[var] <- round(summary(model)$r.squared,2)
...
#also see R^2 for Abdomen - only model:
model_simple <- lm(BODYFAT ~ ABDOMEN, data=data_use_new)
